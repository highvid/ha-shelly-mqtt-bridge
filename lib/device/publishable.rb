module Device
  module Publishable
    ...
    def announce_multiple_topics!
      actions = @announce_topics.map do |topic, metas|
        metas.map do |meta|
      ...
      destroy_announcement_client!
      actions
    end

    def announce_single_topic!(topic:, payload:, listen_topic:, announce_method_process:)
      announcement_client.subscribe(listen_topic)
      trigger_announce(topic:, payload:)
      thread = announcement_thread(topic, payload, listen_topic)
      thread.join
      announcement_client.unsubscribe(listen_topic)
    ...
    def announcement_thread(topic, payload, listen_topic)
      ...
      @announcement_threads[topic_payload] = Thread.new do
        Timeout.timeout(5) do
          got_topic, message = announcement_client.get
          AppLogger.warn("Got message from #{got_topic}") if got_topic != listen_topic
